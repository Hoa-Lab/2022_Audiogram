...
#---------------------------------------------------------
fd_out='./out/a03_bc_01_plt-case'
f_in='./out/a03_bc_00_clean/data_case.csv'
...
#--------------------------------------------------------
def plt_box(df, f_out, title=None, sz=(4.5,3), cmap=cmap, ylbl='Abs Difference (Db)'):
	sns.set()
	sns.despine()
	#plot
...
l_df=[]
for freq in l_freq:
	dfi=df.loc[:, ['side', f'bcr_{freq}', f'bcl_{freq}']].copy()
	dfi['diff']=dfi[f'bcr_{freq}']-dfi[f'bcl_{freq}']
...
title='Difference of Db (Bone Conduct)'
