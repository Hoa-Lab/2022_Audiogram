...
###################################################################
#load
l0=[f'acr_{i}' for i in l_freq]
l1=[f'acl_{i}' for i in l_freq]
l_col=['side']+l0+l1
...
df0.columns=['side']+[f'ac_{i}' for i in l_freq]

df1=df.loc[:, ['side']+l1].copy()
df1.columns=['side']+[f'ac_{i}' for i in l_freq]
