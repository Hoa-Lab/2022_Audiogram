...
#---------------------------------------------------------
fd_out='./out/a03_bc_02_plt-ear'
f_in='./out/a03_bc_00_clean/data_ear.csv'
...
#-------------------------------------------------------
def plt_box(df, f_out, title=None, sz=(4.5,3), cmap=cmap, ylbl='Db'):
	sns.set()
	sns.despine()
	#plot
	fig, ax=plt.subplots(figsize=sz)
	ax=sns.boxplot(x='freq', y='bc', hue='tumor', data=df, palette=cmap, showfliers=False, linewidth=0.5)
	#adjust
...
l_df=[]
for freq in l_freq:
	dfi=df.loc[:, [f'bc_{freq}', 'tumor']].copy()
	dfi.columns=['bc', 'tumor']
	dfi['freq']=f'bc_{freq}'
	l_df.append(dfi)
df=pd.concat(l_df)
