...
fd_out='./out/a01_wordrec_00_clean'
f_in=f'{prj}/b00_clean/out/a00_clean_06_merge/data.csv'
l_col=['side', 'wordrec_r', 'wordrec_l']
...
#by ear --- clean
df0=df.loc[:, ['side', 'wordrec_r']].copy()
df0.columns=['side', 'wordrec']

df1=df.loc[:, ['side', 'wordrec_l']].copy()
df1.columns=['side', 'wordrec']
